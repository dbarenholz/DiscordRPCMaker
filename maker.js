 ...
 */
 ...
 console.log(
   chalk`{yellow Did you create an application at} {blue https://discord.com/developers/applications}{yellow ? If not, you probably should read the} {green README} {yellow on the GitHub repo before continuing. Enter} {green 'y'} {yellow or} {green 'yes'} {yellow to continue.\n}`
 );
 ...
 while (!isnum) {
   console.log(
	 chalk`{underline What is your app's {bold.green Client ID?} This can be found on the main section of your app.}\n`
   );
   clientid = prompt();
 ...
 largeimage = prompt();
 if (largeimage !== "") {
   console.log(chalk`{underline \nWhat's the text of the {bold.green larger image}?}\n`);
   largeimagetext = prompt();
   console.log(chalk`\n{underline What's the name of the {bold.green smaller image}?}\n`);
   smallimage = prompt();
   if (smallimage !== "") {
	 console.log(chalk`\n{underline What's the text of the {bold.green smaller image}?}\n`);
	 smallimagetext = prompt();
   }
 }
 ...
 if (numbuttons >= 1) {
   ...
   while (!isurl) {
	 ...
	 buttononeurl = prompt();
	 if (!validateurl(buttononeurl)) {
	   console.log(chalk`{red That's not a website!}`);
	 } else {
	   isurl = true;
	 }
   }
 ...
 Large Image Text: {green.bold ${largeimagetext}}
 Small Image Asset: {green.bold ${smallimage}}
 Small Image Asset: {green.bold ${smallimagetext}}
